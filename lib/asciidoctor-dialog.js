...
/**
 * 
 * Находит есть ли около найденного меньшего термина - больший
 *
...
function replaceBySplitString(doc, output) {
  ...
  let terminsTable = blockById(doc);
  ...
  parsedOutput.querySelectorAll("p:not(table#tnc p)").forEach((e) => {
    let words = e.innerText.split(" ");
    var lastReplacableIndexInInnerHTML = 0;
    for (let i = 0; i < words.length; i++) {
      for (let t = 0; t < terminsDictionary.length; t++) {
        let terminWords = terminsDictionary[t].termin.split(" ");
        ...
        if (terminWords.length > 1) {
          ...
          if (words[i].replace(/(^,)|(,$)/g, "") === terminsDictionary[t].termin) {
            firstCaptureResult = true;
          }
        } else {
          continue
        ...
        if (firstCaptureResult) {
          ...
          if (!findBigger) {
            // console.log(`Нашел ${terminsDictionary[t].termin} в ${words}`);
            capturedTermin = terminsDictionary[t];
          }
          // console.log(capturedTermin);


          let firstWordIntermin = capturedTermin.termin.split(" ")[0];
          let lastWordInTermin = capturedTermin.termin.split(" ")[capturedTermin.termin.split(" ").length-1];
          ...
          // Вставить </abbr> в lIndex в innerHTML
          // console.log(`innerHTML: ${e.innerHTML}\nfSlice: ${e.innerHTML.slice(0, lIndex)}\nlSlice: ${e.innerHTML.slice(lIndex)}`)
          // console.log(`before: ${e.innerHTML.slice(0, lIndex)}\nafter: ${e.innerHTML.slice(lIndex)}`)
          // e.innerHTML = [e.innerHTML.slice(0, lIndex), "</abbr>", e.innerHTML.slice(lIndex)].join("");
          // Вставить <abbr title="capturedTermin.description"> в fIndex в innerHTML
          ...
          // lastReplacableIndexInInnerHTML = lastIndexOf("</abbr>")
          lastReplacableIndexInInnerHTML = e.innerHTML.lastIndexOf("</abbr>")
          // console.log(`result innerHTML: ${e.innerHTML}\ninserIndex: ${lastReplacableIndexInInnerHTML}`)
          break
        }
      }
    }
    // console.log("Я тут")
    // Пользователь обращается через телефон до <abbr title=\"Пользователь из Ульяновска\">ПИУ</abbr>, а ему отвечает ПИ
